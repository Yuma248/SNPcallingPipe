...
while (@ARGV){
	...
	if ($_=~ /^-stp$/){$stp=shift @ARGV;}
 	...
 	elsif ($_=~ /^-rbar$/){$rbar=shift @ARGV;}
 	elsif ($_=~ /^-dd$/){$dd=shift @ARGV;}
...
foreach $stp (@stptr){
	if ($stp eq "indref"){
		...
		our @arg = ("-i $input","-o $output","-bf $barcode_file","-lnc $lnc","-snc $snc", "-rad $radtag", "-dd $dd");
		dDocent::demul(@arg);
		$stprn = 1; 
	}
		...
		if (not defined ($exf)){$exf ="1.fq.gz,2.fq.gz";}
  		if (not defined ($rbar)){$rbar = "n";}
   		...
   		if ($rbar eq "y"){our @arg = ("-i $input","-o $output","-fm $fm","-nc $snc","-exf $exf","-rbar $rbar", "-bf $barcode_file");}
     		else {our @arg = ("-i $input","-o $output","-fm $fm","-nc $snc","-exf $exf");}
	        RemAdap::trim(@arg);
        	$stprn = 2;
	}
