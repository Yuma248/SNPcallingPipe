...
#`parallel -j $nct mv $inf/{1}.sam /scratch/user/reev0074/merged/ ::: @nms`;
`parallel -j $nc samtools fixmate -m $names/{1}_namesort.bam $fix/{1}_fixmate.bam ::: @nms`;
#`parallel -j $nct mv $names/{1}_namesort.bam /scratch/user/reev0074/merged/ ::: @nms`;
`parallel -j $nc samtools sort -\@ $ncp -o $coor/{1}_positionsort.bam $fix/{1}_fixmate.bam ::: @nms`;
#`parallel -j $nct mv $fix/{1}_fixmate.bam /scratch/user/reev0074/merged/ ::: @nms`;
`parallel -j $nc --results $tmpdir samtools markdup -r -s $coor/{1}_positionsort.bam $ddp/{1}_markdup.bam ::: @nms`; 
#`parallel -j $nct mv $coor/{1}_positionsort.bam /scratch/user/reev0074/merged/ ::: @nms`;
